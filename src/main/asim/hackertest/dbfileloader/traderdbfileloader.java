    ...
    public List<Trader> traderfile() throws IllegalArgumentException, FileSystemNotFoundException, SecurityException, URISyntaxException {
		...
		/*
		   ...
		   to Path to work. This issue would not occur if the implementation simply used URL#openStream(), which
		    ...
		    URI uriPath = getClass().getResource("/trader-data.txt").toURI();
			try(Stream<String> traderFileLines = Files.lines(AppUtility.getResourcePath(uriPath)))
		 */
        InputStream iStream = getClass().getResourceAsStream("/trader-data.txt");
        BufferedReader bReader = new BufferedReader(new InputStreamReader(iStream));
        try {
			String traderFileLine = null;
            while ((traderFileLine = bReader.readLine()) != null){
                ...
                List<String[]> tokens = Stream.of(traderFileLine).map(line -> line.split(",")).collect(Collectors.toList());
